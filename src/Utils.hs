...
compileExercise config (ExerciseInfo _ exDirectory exFilename exIsRunnable _) = do
  ...
  let fullSourcePath = (root ++ exercisesExt config ++ exDirectory) `pathJoin` exFilename
...
fullExerciseFp projectRoot exercisesExt (ExerciseInfo _ exDir exFile _ _) = (projectRoot ++ exercisesExt ++ exDir) `pathJoin` exFile
