...
applyAndCountOperations :: [Op] -> Double -> (Double, Int)
...
opList2 :: [Op]
opList2 = [Multiply 2.0, Add 5.0, Sqrt]

main :: IO ()
main = defaultMain $ testGroup "Monads4" $
  [ testCase "applyOpLog 1" $ runWriter (applyOpLog (Add 1) 5.0) @?= (6.0, IntAdd 1)
  , testCase "applyOpLog 2" $ runWriter (applyOpLog (Multiply 2) 5.0) @?= (10.0, IntAdd 5)
  , testCase "applyOpLog 3" $ runWriter (applyOpLog (Divide 2) 6.0) @?= (3.0, IntAdd 10)
  , testCase "applyOpLog 4" $ runWriter (applyOpLog Sqrt 4.0) @?= (2.0, IntAdd 20)
  ...
  , testCase "applyOpCount 1" $ runWriter (applyOpCount (Add 1) 5.0) @?= (6.0, ["Adding 1"])
  , testCase "applyOpCount 2" $ runWriter (applyOpCount (Multiply 2) 5.0) @?= (10.0, ["Multiplying by 2"])
  , testCase "applyOpCount 3" $ runWriter (applyOpCount (Divide 2) 6.0) @?= (3.0, ["Dividing by 2"])
  , testCase "applyOpCount 4" $ runWriter (applyOpCount Sqrt 4.0) @?= (2.0, ["Taking Square Root"])
